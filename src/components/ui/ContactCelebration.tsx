...
import { CONTACT_CELEBRATION_EVENT } from "@/lib/contactCelebration";
...
export default function ContactCelebration() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(CONTACT_CELEBRATION_EVENT, handleCelebration);
    return () => {
      window.removeEventListener(CONTACT_CELEBRATION_EVENT, handleCelebration);
    };
  }, []);
            ...
            {burst.particles.map((particle: any) => (
              <motion.span
                ...
                animate={{
                  opacity: [0, 1, 1, 0],
                  scale: [0.2, 1, 0.9, 0.2],
                  x: [0, particle.dx * 0.7, particle.dx],
                  y: [0, particle.dy * 0.7, particle.dy],
                }}
                transition={{
